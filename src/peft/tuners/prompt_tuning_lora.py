...
class PromptEmbeddingLoRA(torch.nn.Module):
    ...
    **Attributes**:
    ...
    def __init__(self, config, word_embeddings):
        ...
        self.lora_embedding_A = nn.Parameter(self.weight.new_zeros((config.max_length, config.r)))
        self.lora_embedding_B = nn.Parameter(self.weight.new_zeros((config.r, config.hidden_size)))
        self.scaling = config.lora_alpha / config.r
        if config.init_lora_weights:
            nn.init.kaiming_uniform_(self.lora_embedding_A.weight, a=math.sqrt(5))
            nn.init.zeros_(self.lora_embedding_B.weight)
